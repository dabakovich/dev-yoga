...
- `scaffold` Executed plan 02: added read-only `list_tasks` tool (compact shape, truncated descriptions) and a PLAN-THE-DAY prompt section weighing priority + status + age; verified live — an in_progress low task correctly outranked a medium todo.
- `scaffold` Executed plan 03: added `update_task`/`delete_task` tools with id-resolution-via-`list_tasks` and a hard delete-confirm gate; mobile refreshes the list and shows Updated/Deleted confirmations; verified live — direct status flip, two-turn delete, ambiguous reference triggers a question.
- `edit` Added long-press copy context menu to chat message bubbles: `expo-clipboard` + `@expo/ui/swift-ui` `ContextMenu` on iOS; `Pressable` + `ToastAndroid` fallback on Android.
- `scaffold` Executed plan 06 (agent memory): `MemoryFact` entity + `AgentMemoryService` (in `AiModule`, not tasks), `buildSystemPrompt(facts)` folding durable facts into the prompt, `remember`/`forget` tools, and `savedMemories`/`forgotMemories` effects surfaced in the mobile chat; onboarding nudge when memory is empty.
- `fix` `forget` tool let the model fabricate success: soft phrasing ("forget the Friday thing") picked a match phrase absent from the stored fact, matched nothing, yet the model still claimed removal — switched to word-overlap matching and made a no-match return the current facts so it can't lie.
- `review` Verified memory end-to-end against the live Anthropic key: remember saves, dedupe skips a restatement, a fresh transcript recalls the fact, and forget removes the right one; documented memory's shape and limits (flat, no retrieval, single user) in the backend README.
- `scaffold` Submission docs pass: agent read the brief PDF and wrote the root README — quick start (.env setup, seed, LAN-IP real-device testing), architecture + SQLite trade-offs, AI features mapped to the brief, David's decisions, descoped ideas, and the agentic-first development principles.
...
- `edit` Task 8: replaced chat-api with conversation-aware RTK Query endpoints (list/get/delete/sendChat + Conversation tags); retargeted formatChatReply to MessageEffects, ChatMessageBubble to backend Message type; deleted chat-slice.ts and removed it from store + persist whitelist.
...
- `plan` Adapted the backend-conversations plan's mobile phase (Tasks 7–10) to reuse the now-merged decomposed chat units (`ChatMessageBubble`/`ChatTypingBubble`/`ChatEmptyState`/`ChatInputBar`/`useKeyboardVisible`) instead of re-inlining them — thread screen is pure orchestration; only `ChatMessageBubble` + `formatChatReply` change to consume backend types.
...
- `fix` Root-caused why Maestro `assertVisible` couldn't see task cards: expo-router's native `Link.Preview`/`Link.Menu` host strips its trigger subtree from the iOS accessibility tree (live hierarchy dump showed the FlatList present but zero card nodes — also invisible to VoiceOver). Confirmed `testID`/`accessibilityLabel` on the inner Pressable can't surface it.
- `fix` Re-exposed task cards by wrapping the `<Link>` in an outer `accessible` View (`accessibilityLabel={title}`, `testID="task-<id>"`); keeps tap-nav + long-press preview/Delete menu and fixes VoiceOver. Verified end-to-end with a gitignored repro flow (`mobile/.maestro/repro-link-a11y.yaml`).
- `fix` Replaced task-detail header "Delete" text button with a SwiftUI trash icon (`@expo/ui/swift-ui Button`, label "Delete task") to remove text ambiguity with the Alert's "Delete" button; updated create-task.yaml cleanup to tap "Delete task" then "Delete" — flow is now PASS and self-cleaning.
- `decision` Removed legacy `ios-screenshot` skill — superseded by `maestro-e2e` (which covers static screenshots via `takeScreenshot` and interactive flows); kept both was confusing agents about which to reach for.
